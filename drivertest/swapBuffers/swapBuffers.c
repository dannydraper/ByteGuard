...
//ULONG LoggingFlags = LOGFL_ERRORS | LOGFL_READ | LOGFL_WRITE | LOGFL_DIRCTRL | LOGFL_VOLCTX;             // all disabled by default
...
--*/
{
	...
	LARGE_INTEGER offset = iopb->Parameters.Read.ByteOffset;
    ...
    try {
		...
		//DbgPrint ("Doing a pre-read....Offset: %d Length: %d \n", offset.QuadPart, readLen);

        if (readLen == 0) {
			DbgPrint ("No swapbuffers done - readLen was 0");
            leave;
        } else {
			DbgPrint ("Offset Highpart: %d, LowPart: %d", offset.HighPart, offset.LowPart);

			if (offset.QuadPart == 0) {
				DbgPrint ("PreRead offset was zero! ");
				DbgPrint (" Readlen is: %d\n", readLen);
			} else {
				DbgPrint ("PreRead Offset is: %ld", offset.QuadPart);
				DbgPrint (" Readlen is: %d\n", readLen);
			}

			//DbgPrint ("Proceeding... - readLen was greater than 0");
...
--*/
{
	...
	//DbgPrint ("Doing a post-read....Offset: %i, Length: %i\n", offset.QuadPart, readLen);
    ...
    try {
		...
		if (RtlUnicodeStringToAnsiString (&aFilename, &targetFile->FileName, TRUE) == STATUS_SUCCESS) {
			DbgPrint ("SwapBuffers: Ansi: %s ", aFilename.Buffer);
		...
		if (readLen == 0) {
			...
			DbgPrint ("POSTREAD Offset Highpart: %d, LowPart: %d ", offset.HighPart, offset.LowPart);

			if (offset.QuadPart == 0) {
				DbgPrint ("POSTREAD offset was zero! ");
				DbgPrint (" Readlen is: %d\n", readLen);
			} else {
				DbgPrint ("POSTREAD Offset is: %ld", offset.QuadPart);
				DbgPrint (" Readlen is: %d\n", readLen);
			}
